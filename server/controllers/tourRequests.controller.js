...
function roleNameOf(user) {
  return (user?.role_id?.name || user?.role || "")
    .toString()
    .trim()
    .toLowerCase();
}
...
function normalizeLocations(locs = []) {
  return (Array.isArray(locs) ? locs : [])
    .map((x, i) => ({
      ...
      order: Number.isInteger(x.order) ? x.order : i,
    }))
...
export const submitTourRequest = async (req, res) => {
  try {
    if (roleNameOf(req.user) !== "guide") {
      return res
        .status(403)
        .json({ message: "Chỉ HDV mới được gửi yêu cầu tạo tour." });
    }
    ...
    const doc = await TourRequest.create({
      ...
      duration_hours:
        typeof duration_hours !== "undefined"
          ? duration_hours === null
            ? null
      ...
      min_days_before_start: Number.isInteger(min_days_before_start)
        ? min_days_before_start
        : 0,
      max_days_advance: Number.isInteger(max_days_advance)
        ? max_days_advance
        : 180,
    ...
    return res
      .status(201)
      .json({
        message: "Đã gửi yêu cầu tạo tour. Vui lòng chờ duyệt.",
        request_id: doc._id,
      });
  } catch (err) {
  ...
  try {
    ...
    if (err && err.name === "StrictPopulateError") {
      return res
        .status(500)
        .json({
          message: "Populate failed: check populated fields against schema.",
          detail: err.message,
        });
    }
    return res.status(500).json({ message: "Lỗi máy chủ." });
        ...
        [
          {
            ...
            duration:
              r.duration ||
              Math.max(1, Math.ceil((r.duration_hours ?? 24) / 24)),
            // store duration_hours explicitly if provided
            duration_hours:
              typeof r.duration_hours !== "undefined"
            ...
            created_by:
              r.created_by ||
              (Array.isArray(r.guides) && r.guides[0]
                ? r.guides[0].guideId
                : null),
            created_by_role: "guide",

            // Set approval status to approved since this is from approved request
            approval: {
              status: "approved",
              reviewed_by: req.user ? req.user._id : null,
            ...
            min_days_before_start: Number.isInteger(r.min_days_before_start)
              ? r.min_days_before_start
              : 0,
            max_days_advance: Number.isInteger(r.max_days_advance)
              ? r.max_days_advance
              : 180,
            closed_weekdays: Array.isArray(r.closed_weekdays)
              ? r.closed_weekdays
              : [],
            blackout_dates: Array.isArray(r.blackout_dates)
              ? r.blackout_dates
              : [],
...
export const listMyTourRequests = async (req, res) => {
  try {
    if (roleNameOf(req.user) !== "guide")
      return res.status(403).json({ message: "Chỉ HDV." });
    const { status, page = 1, limit = 12 } = req.query;

    const filter = {
      $or: [{ created_by: req.user._id }, { "guides.guideId": req.user._id }],
    };
...
export const updateTourRequest = async (req, res) => {
  try {
    ...
    if (r.status !== "pending")
      return res
        .status(409)
        .json({ message: "Chỉ có thể cập nhật khi trạng thái pending." });
    if (String(r.created_by) !== String(req.user._id))
      return res
        .status(403)
        .json({ message: "Bạn không có quyền sửa yêu cầu này." });
...
export const deleteTourRequest = async (req, res) => {
  try {
    ...
    if (String(r.created_by) !== String(req.user._id))
      return res
        .status(403)
        .json({ message: "Bạn không có quyền xóa yêu cầu này." });
    if (r.status !== "pending")
      return res
        .status(409)
        .json({ message: "Chỉ có thể xóa khi trạng thái pending." });
