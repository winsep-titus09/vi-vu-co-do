...
export const getMyBusyDates = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Lỗi máy chủ khi lấy danh sách ngày bận." });
  }
};
...
export const addBusyDates = async (req, res) => {
  try {
    ...
    if (!dateList.length) {
      return res
        .status(400)
        .json({ message: "Vui lòng cung cấp ngày cần đánh dấu bận." });
    }
    ...
    for (const d of dateList) {
      try {
        // Parse date string và tạo UTC date để tránh timezone issues
        ...
        if (err.code === 11000) {
          results.skipped.push({
            date: d,
            reason: "Ngày đã được đánh dấu bận.",
          });
        } else {
    ...
    return res
      .status(500)
      .json({ message: "Lỗi máy chủ khi đánh dấu ngày bận." });
  }
};
...
export const removeBusyDates = async (req, res) => {
  try {
    ...
    if (!dateList.length) {
      return res
        .status(400)
        .json({ message: "Vui lòng cung cấp ngày cần xóa." });
    }

    // Tạo query conditions để match cả dữ liệu cũ (local timezone) và mới (UTC)
    const dateConditions = dateList.map((d) => {
      let year, month, day;
      if (typeof d === "string" && /^\d{4}-\d{2}-\d{2}$/.test(d)) {
      ...
      // UTC midnight (dữ liệu mới)
      const utcDate = new Date(Date.UTC(year, month - 1, day, 0, 0, 0, 0));
      // Start và end của ngày đó theo UTC (để bắt dữ liệu cũ lưu theo local timezone)
      const startOfDay = new Date(
        Date.UTC(year, month - 1, day - 1, 17, 0, 0, 0)
      ); // 17:00 UTC ngày trước = 00:00 UTC+7
      const endOfDay = new Date(
        Date.UTC(year, month - 1, day, 16, 59, 59, 999)
      ); // 16:59 UTC = 23:59 UTC+7

      return {
        $or: [
          { date: utcDate }, // Match dữ liệu mới (UTC midnight)
          { date: { $gte: startOfDay, $lte: endOfDay } }, // Match dữ liệu cũ (local timezone)
        ],
      };
    });
    ...
    const result = await GuideBusyDate.deleteMany({
      guide_id: guideId,
      $or: dateConditions.map((c) => c.$or).flat(),
    });
...
export const getAvailableGuides = async (req, res) => {
  try {
    ...
    if (!date) {
      return res
        .status(400)
        .json({ message: "Vui lòng cung cấp ngày (date)." });
    }
...
export const getGuideCalendar = async (req, res) => {
  try {
    ...
    const tours = await Tour.find({
      $or: [{ guide_id: guideId }, { "guides.guideId": guideId }],
      status: "active",
      "approval.status": "approved",
    })
      .select(
        "name slug duration duration_hours blackout_dates closed_weekdays"
      )
      .lean();
...
export const getTourCalendar = async (req, res) => {
  try {
    ...
    const busyDates = await GuideBusyDate.find({
      guide_id: {
        $in: uniqueGuideIds.map((id) => new mongoose.Types.ObjectId(id)),
      },
      date: { $gte: startOfMonth, $lte: endOfMonth },
    })
