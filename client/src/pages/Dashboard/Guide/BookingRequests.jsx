...
const allRequests = [
  {
    ...
    note: "Gia đình có người già, xin vui lòng đi chậm.",
  ...
  },
  {
    ...
    note: "No spicy food please.",
    requestTime: "5 giờ trước",
    status: "pending",
  },
  {
    id: "BK-880",
  ...
  {
    id: "BK-850",
    tourName: "Khám phá phá Tam Giang",
    tourist: { name: "Trần Tuấn", avatar: null },
    date: "10/05/2025",
    time: "14:00 PM",
    guests: 10,
    totalPrice: "5.000.000đ",
    note: "Đoàn đông cần xe đưa đón.",
    requestTime: "2 tuần trước",
    status: "rejected",
  },
];
...
export default function BookingRequests() {
  ...
  const filteredRequests = allRequests.filter((req) => {
    ...
    const matchSearch =
      req.tourist.name.toLowerCase().includes(search.toLowerCase()) ||
      req.tourName.toLowerCase().includes(search.toLowerCase());
              ...
              <div className="flex flex-col lg:flex-row gap-6">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-2 gap-x-4 text-sm text-text-secondary mb-4">
                    ...
                    <div className="flex items-center gap-2">
                      <IconClock className="w-4 h-4 text-primary" /> {req.time}
                    </div>
                    <div className="flex items-center gap-2">
                      <IconUser className="w-4 h-4 text-primary" /> {req.guests}{" "}
                      khách
                    </div>
                  </div>
                  ...
                  <div className="w-full mt-auto text-right">
                    ...
                    {req.status === "pending" && (
                      <div className="flex flex-col gap-2">
                        <button className="w-full px-4 py-2 rounded-xl bg-primary text-white font-bold text-sm hover:bg-primary/90 shadow-lg shadow-primary/20 transition-all flex items-center justify-center gap-2 whitespace-nowrap">
                          <IconCheck className="w-4 h-4" /> Chấp nhận
                        </button>
                        <button className="w-full px-4 py-2 rounded-xl border border-red-100 text-red-600 bg-red-50 hover:bg-red-100 font-bold text-sm transition-colors whitespace-nowrap">
                          Từ chối
                        </button>
                      </div>
                    )}
